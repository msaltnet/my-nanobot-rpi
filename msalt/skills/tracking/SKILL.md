...
**날짜를 정하기 전에 현재 대화의 직전 추적 질문을 먼저 확인한다.** 특히 사용자가
`"5시간"`, `"했어"`, `"2잔"`처럼 항목·날짜를 생략한 단답을 보냈다면, 가장 최근에
답을 기다리던 질문에서 항목과 대상 날짜를 가져온다. 질문의 `대상 날짜: YYYY-MM-DD`,
`YYYY-MM-DD 기록`, 묶음 알림의 항목별 날짜가 기록 대상이며, 날짜를 오늘이나 어제로
다시 계산하지 않는다. 예: `"수면 기록할 시간이야. 대상 날짜: 2026-09-14."`에
`"5시간"`이라고 답하면 `수면 --date 2026-09-14 --num 300`으로 기록한다.

사용자가 **이번 답변에** 항목이나 날짜를 명시하면 그 명시가 이전 질문보다 우선한다.
직전 질문을 확인할 수 없고 답변에도 날짜·항목이 없어 기록 대상을 확정할 수 없으면
추측해서 저장하지 말고 필요한 정보 하나만 묻는다.
